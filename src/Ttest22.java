...
public class Ttest22 {
	...
	//2018-08  2018-01
	private static List<String> getDay(String minDate, String maxDate) {
		//判断输入的最小值 是不是  当前月  true 就是
		if(Ttest22.isCurrenMonth(minDate)) {
			return getDayListOfMonth();
		}
		
		//如果最大值是当前月
		if(Ttest22.isCurrenMonth(maxDate)) {
			String str = Ttest22.getParamMap();
			List<String> strList = Ttest22.getMonthBetween(minDate, str);
			List<String> dayList = getDayListOfMonth();
			strList.addAll(dayList);
			return strList;
		}
		
		List<String> strList = Ttest22.getMonthBetween(minDate, maxDate);
		return strList;
	...
	public static void main(String[] args) {
		...
		//System.out.println(Ttest22.getUserDate("2018-6-1"));
		
		List<String> ss = Ttest22.getFormatDate(Ttest22.getDay("2018-05", "2018-08"));
		for(String s1 : ss) {
